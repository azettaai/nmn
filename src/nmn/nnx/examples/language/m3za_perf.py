...
class ModernTransformerBlock(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.attn = RotaryYatAttention(
            ...
            use_performer=True,
            performer_num_features=embed_dim // 2,  # Half the dim for efficiency
        ...
        self.ffn = nnx.Sequential(
            YatNMN(
                embed_dim, ff_dim, kernel_init=kernel_init, rngs=rngs, use_bias=False
            ),
            nnx.Linear(
                ff_dim, embed_dim, kernel_init=kernel_init, rngs=rngs, use_bias=False
            ),
        )
    ...
    def __call__(self, x, training: bool = False):
        ...
        h = x
        attn_out = self.attn(
            h, deterministic=not training, rngs=self.rngs if training else None
        )
...
class MiniBERT(nnx.Module):
        ...
        self.transformer_blocks = nnx.List(
            [
                ModernTransformerBlock(
                    embed_dim, num_heads, feed_forward_dim, maxlen=maxlen, rngs=rngs
                )
    ...
    def __call__(self, inputs, training: bool = False):
        ...
        embedding_weights = self.embedding_layer.token_emb.embedding.get_value()
...
def create_masked_lm_predictions(
    tokens, mask_prob, maxlen, vocab_size, mask_token_id, pad_token_id
):
    ...
    masked_indices = np.random.choice(
        non_padding_indices,
        size=min(num_to_predict, len(non_padding_indices)),
        replace=False,
    )
    ...
    for i in masked_indices:
        ...
        if rand < 0.8:
            ...
            tokens[i] = np.random.randint(0, vocab_size)
    return tokens, labels
...
def process_dataset_for_mlm(dataset, tokenizer, maxlen, mask_prob, vocab_size):
    ...
    def tokenize_pad_and_mask(examples):
        ...
        for text in examples["text"]:
            ...
            masked, label = create_masked_lm_predictions(
                token_array.copy(),
                mask_prob,
                maxlen,
                vocab_size,
...
def process_dataset_for_contrastive(dataset, tokenizer, maxlen):
    ...
    dataset = dataset.map(
        tokenize_pairs, batched=True, batch_size=1000, remove_columns=columns_to_remove
    )
    return dataset.shuffle(buffer_size=10_000, seed=42)
...
def train_step_mlm(model: MiniBERT, optimizer: nnx.Optimizer, batch):
    grad_fn = nnx.value_and_grad(
        lambda m, b: loss_fn_mlm(m, b, training=True), has_aux=True
    )
...
def contrastive_loss_fn(model, batch, training: bool, temperature: float = 0.05):
    ...
    loss = optax.softmax_cross_entropy_with_integer_labels(
        logits=sim_matrix, labels=labels
    )
...
def train_step_contrastive(model: MiniBERT, optimizer: nnx.Optimizer, batch):
    grad_fn = nnx.value_and_grad(
        lambda m, b: contrastive_loss_fn(m, b, training=True), has_aux=False
    )
...
class MiniBERTForMTEB:
    ...
    def encode(self, sentences, task_name=None, prompt_name=None, **kwargs):
        ...
        def process_batch_texts(text_batch):
            ...
            pooled = np.sum(np.asarray(hidden_state) * mask_exp, 1) / np.clip(
                mask_exp.sum(1), a_min=1e-9, a_max=None
            )
...
def main_pretrain():
    ...
    train_dataset = process_dataset_for_mlm(
        full_dataset.skip(config["val_set_size"]),
        tokenizer,
        config["maxlen"],
        config["mask_prob"],
        config["vocab_size"],
    )
    val_dataset = process_dataset_for_mlm(
        full_dataset.take(config["val_set_size"]),
        tokenizer,
        config["maxlen"],
        config["mask_prob"],
        config["vocab_size"],
    ...
    val_iterator = iter(
        val_dataset.iter(batch_size=config["batch_size"], drop_last_batch=True)
    )
    ...
    for step in range(max_iterations):
        ...
        sharded_batch = {
            k: jax.device_put(jnp.array(v), sharding) for k, v in batch.items()
        }
...
def main_contrastive_tuning(mlm_checkpoint_path, config):
    ...
    wandb.init(
        project=config["wandb_project"], config=config, name="phase2_contrastive"
    )
    ...
    optimizer = nnx.Optimizer(
        model, optax.adamw(config["contrastive_lr"]), wrt=nnx.Param
    )
    ...
    train_iterator = iter(
        contrastive_dataset.iter(
            batch_size=config["contrastive_batch_size"], drop_last_batch=True
        )
    )
    ...
    for step in range(config["contrastive_steps"]):
        ...
        if (step + 1) % 100 == 0:
            print(
                f"Contrastive Step {step+1}/{config['contrastive_steps']}, Loss: {loss.item():.4f}, Time: {time.time()-start_time:.2f}s"
            )
            start_time = time.time()
...
def main_eval(checkpoint_path, config):
    ...
    mteb_model = MiniBERTForMTEB(
        model=model, tokenizer=tokenizer, maxlen=config["maxlen"]
    )
